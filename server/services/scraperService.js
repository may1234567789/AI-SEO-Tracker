...
export async function scrapeUrl(url) {
    ...
    try {
        ...
        const defaultContent = browser.contexts()[0];
        pageXOffset.setDefaultNavigationTimeout(30000);
        ...
        //Exteract all SEO relevent data from the rendered page
        const scrapedData = await page.evaluate(() => {
            ...
            const h1Texts = Array.from(h1Elements).map((el) => el.textContents?.trim() || "");
            const headings = {
            ...
            return {
                ...
                headings, links: { internal: internalLinks, external: externalLinks, total: allLinks.length },
                images: { total: allImages.length, missingAlt, withAlt: allImages.length - missingAlt }, wordCount, pageSize, bodyText: bodyText.substring(0, 3000),
            }
        })
        ...
        console.error("[SCRAPER] Playeright session failed: ", error.message);
        if (browser) {
            try {
                await browser.close()
            } catch (error) {
